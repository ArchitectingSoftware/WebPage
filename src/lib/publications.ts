export const PublicationsList = [
    {
        ...
        APIs that can make differentiating between normal and malicious \
        behavior difficult to classify. The nature of managing every aspect of \
        cloud native systems is prone to misconfiguration errors, requiring \
        the setting and auditing of thousands of parameters in enterprise \
        systems. Successful data breaches require bad actors to remain \
        undetected for long periods of time so that data exfiltration does \
        not trigger intrusion protection controls. In this paper we recreate \
        a realistic cloud native environment to emulate a data breach attack \
        using common API misconfiguration mistakes. We then introduce \
    ...
    },
    {
        ...
        as a Service (FaaS) platforms, or managed container orchestration \
        runtimes such as Kubernetes. These systems are difficult to monitor \
        for correct operation because the many parts of the application can \
        be terminated or (re)started at any time in the face of errors for \
        resiliency, or for the economic benefits associated with autoscaling. \
        When security vulnerabilities are discovered, the impacted \
        runtime components need to be quickly monitored, identified and \
        patched. This paper introduces a platform we created to observe \
        and measure the health of cloud-native applications by applying \
        machine learning techniques that benchmark normal behavior and \
        can detect when the behavior drifts away from the benchmark due \
        to security attacks. To achieve this goal, our platform must be able \
